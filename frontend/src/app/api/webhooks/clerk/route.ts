...
export async function POST(request: Request) {
  ...
  if (evt.type == 'session.created') {
    ...
    const timeDiffSeconds = Math.abs((sessionCreatedAt.getTime() - userCreatedAt.getTime()) / 1000);
    ...
    if (timeDiffSeconds < 10) {
      ...
      return NextResponse.json({ message: 'New user with first session processed' });
    }
